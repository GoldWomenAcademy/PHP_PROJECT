<?php
/* Pas obligatoire les parenthèses avec la function echo,  fonctionne avec de simple côtes.
Son rôle de renvoyer à l'écran qui est envoyé en paramètre par la function ici 'bonjour'
Instruction avec simple ou double cote fonctionne 
Avant une apostrophe mettre un anti slash 
*/
echo 'bonjour j\'aime le php';
/* Double anti slash pour mettre un mot entre guillets
un au début du mot ouverture des guillets mot anti slash à la fin du mot et fermeture des guillets*/
echo 'bonjour \"jaime\" le PHP';

/* 
Function print
*/

print 

?>